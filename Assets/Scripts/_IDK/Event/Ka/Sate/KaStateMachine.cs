...
public class KaStateMachine : MonoBehaviour
{
    ...
    [SerializeField] private bool useUniqueDebugColor = true;
    [SerializeField] private bool offsetDebugGizmos = true;
    [SerializeField] private float debugGizmoHeightStep = 0.12f;
    [SerializeField, Range(3, 32)] private int sightConeSegments = 12;
    [SerializeField, Range(8, 64)] private int attackAreaSegments = 24;

    private KaState currentState;
    private KaIdle idleState;
    private KaPatrol patrolState;
    private KaChase chaseState;
    private KaAttack attackState;
    private KaSearch searchState;
    private KaExecute executeState;
    private bool warnedMissingAgent;
    private bool warnedMissingPlayer;
    private Vector3 lastSeenPlayerPosition;
    private bool hasLastSeenPlayerPosition;
    private float movementSpeedMultiplier = 1f;
    private float currentBaseAgentSpeed;
    ...
    public Vector3 DebugGizmoOffset => GetDebugGizmoOffset();
    ...
    public KaIdle IdleState => idleState;
    public KaPatrol PatrolState => patrolState;
    public KaChase ChaseState => chaseState;
    public KaAttack AttackState => attackState;
    public KaSearch SearchState => searchState;
    public KaExecute ExecuteState => executeState;
    ...
    private void Awake()
    {
        ...
        {
            _audioManager = GetComponent<GhostAudioManager>();
        }

        TryFindPlayer();
        idleState    = new KaIdle(this);
        patrolState  = new KaPatrol(this);
        chaseState   = new KaChase(this);
        attackState  = new KaAttack(this);
        searchState  = new KaSearch(this);
        executeState = new KaExecute(this);
    }
    ...
    public void ChangeState(KaState nextState)
    {
        if (nextState == null)
        {
            Debug.LogWarning($"{nameof(KaStateMachine)} on {name} tried to change to a missing state.");
            return;
        }
    ...
    public bool TrySetReachableDestination(Vector3 destination)
    {
        ...
        if (!TryGetReachableNavMeshPosition(destination, out Vector3 navMeshPosition))
        {
            return false;
        }
    ...
    public void PlayAttackJumpscare()
    {
    ...
    {
        if (!enableSight || !EnsurePlayerTarget())
        {
            return false;
        }
        ...
        if (toPlayer.sqrMagnitude > checkRange * checkRange)
        {
            return false;
        }
    ...
    public void SetSpeedMultiplier(float speedMultiplier)
    {
        ...
        if (agent != null && currentBaseAgentSpeed > 0f)
        {
            agent.speed = currentBaseAgentSpeed * movementSpeedMultiplier;
        }
    }
    ...
    public void DrawSightCone(Color color, float rangeBonus, float angleBonus)
    {
        ...
        Vector3 offset = GetDebugGizmoOffset();
        ...
        Gizmos.color = GetDebugColor(color);
    ...
    public void DrawAttackArea(Color color)
    {
        ...
        Vector3 offset = GetDebugGizmoOffset();
        Vector3 center = transform.TransformPoint(GetAttackAreaLocalCenter()) + offset;
        float halfWidth = GetAttackAreaHalfWidth();
        float halfDepth = GetAttackAreaHalfDepth();
        ...
        Gizmos.DrawLine(transform.position + offset, center);
    }

    public Color GetDebugColor(Color stateColor)
    {
        if (!useUniqueDebugColor)
        {
            return stateColor;
        }

        float hue = Mathf.Repeat(Mathf.Abs(GetInstanceID()) * 0.1618f, 1f);
        Color instanceColor = Color.HSVToRGB(hue, 0.9f, 1f);
        return Color.Lerp(stateColor, instanceColor, 0.55f);
    }
    ...
    private Vector3 GetDebugGizmoOffset()
    {
        if (!offsetDebugGizmos || debugGizmoHeightStep <= 0f)
        {
            return Vector3.zero;
        }

        int offsetIndex = Mathf.Abs(GetInstanceID()) % 3;
        return Vector3.up * offsetIndex * debugGizmoHeightStep;
    }
    ...
    private float GetAttackAreaHalfWidth()
    {
        if (attackWidth > 0f)
        {
            return attackWidth * 0.5f;
        }

        return Mathf.Max(0.01f, attackRange);
    }
    ...
    private bool CanUseAgent()
    {
        ...
        if (!warnedMissingAgent)
        {
            ...
            Debug.LogWarning($"{nameof(KaStateMachine)} on {name} needs an enabled NavMeshAgent placed on a baked NavMesh.");
        }
    ...
    private void OnDrawGizmos()
    {
        ...
        KaState debugState = currentState ?? patrolState;
    ...
    private bool TryFindPlayer()
    {
        ...
        if (!warnedMissingPlayer)
        {
            ...
            Debug.LogWarning($"{nameof(KaStateMachine)} on {name} needs a Player transform or a GameObject tagged Player.");
        }
